...
from imap_processing.glows import glows_cdf_attrs, __version__
...
def generate_de_dataset(
    ...
    for index, de in enumerate(de_l1a_list):
        ...
        # support_data["flight_software_version"].append(str(de.l0.ccsds_header.VERSION))
        support_data["seq_count_in_pkts_file"].append(int(de.l0.ccsds_header.SRC_SEQ_CTR))
    ...
    for key, value in support_data.items():
        output[key] = xr.DataArray(
            ...
            attrs=dataclasses.replace(glows_cdf_attrs.metadata_attrs,
                                      catdesc=
                                      glows_cdf_attrs.catdesc_fieldname_l1a[key][0],
                                      fieldname=
                                      glows_cdf_attrs.catdesc_fieldname_l1a[key][1],
                                      ).output()
        )
    ...
    for key, value in data_every_second.items():
        output[key] = xr.DataArray(
            ...
            attrs=dataclasses.replace(glows_cdf_attrs.metadata_attrs,
                                      ...
                                      glows_cdf_attrs.catdesc_fieldname_l1a[key][0],
                                      fieldname=
                                      glows_cdf_attrs.catdesc_fieldname_l1a[key][
                                          1]).output(),
        )
...
def generate_histogram_dataset(
    ...
    for index, hist in enumerate(hist_l1a_list):
        ...
        for key in time_metadata.keys():
            time_metadata[key].append(
                hist.__getattribute__(key).to_seconds()
            )
        time_data[index] = epoch_time
