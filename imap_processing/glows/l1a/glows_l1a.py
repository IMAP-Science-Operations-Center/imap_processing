...
def glows_l1a(packet_filepath: Path, data_version: str) -> list[xr.Dataset]:
    ...
    for hist in hist_l0:
        ...
        hist_day = (J2000_TT_EPOCH + met_to_tt_ns(hist.SEC)).astype("datetime64[D]")
...
def process_de_l0(
    ...
    for de in de_l0:
        de_day = (J2000_TT_EPOCH + met_to_tt_ns(de.MET)).astype("datetime64[D]")
...
def generate_de_dataset(
    ...
    for index, de in enumerate(de_l1a_list):
        ...
        epoch_time = met_to_tt_ns(de.l0.MET).astype("datetime64[ns]")
...
def generate_histogram_dataset(
    ...
    for index, hist in enumerate(hist_l1a_list):
        # TODO: Should this be MET?
        epoch_time = met_to_tt_ns(hist.imap_start_time.to_seconds())
