...
class LoAPID(IntEnum):
    ...
    ILO_DIAG_CDH = 721  # Diagnostic bulk HVPS (raw register dumps)
    ILO_DIAG_IFB = 722  # Diagnostic CDH (raw register dumps)
    ILO_DIAG_TOF_BD = 723  # Diagnostic IFB (raw register dumps)
    ILO_DIAG_BULK_HVPS = 724  # Diagnostic PCC (raw register dumps)
    ILO_DIAG_PCC = 725  # Diagnostic TOF Board (raw register dumps)
