...
def test_enumerated(decom_packets_data):
    ...
    for packet in decom_packets_data:
        assert packet.user_data["SC_SWAPI_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_MAG_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_HIT_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_CODICE_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_LO_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_HI_45_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_HI_90_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_ULTRA_45_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_ULTRA_90_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_SWE_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_IDEX_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_GLOWS_STATUS"] == "NOT_OPERATIONAL"
        assert packet.user_data["SC_SPINPERIODVALID"] == "INVALID"
        assert packet.user_data["SC_SPINPHASEVALID"] == "INVALID"
        assert packet.user_data["SC_ATTITUDE"] == "SUNSENSOR"
        assert packet.user_data["SC_CATBEDHEATERFLAG"] == "ON"
        assert packet.user_data["SC_AUTONOMY"] == "OPERATIONAL"
        assert packet.user_data["HIT_STATUS"] == "OFF-NOMINAL"
        assert packet.user_data["SWE_NOM_FLAG"] == "OFF-NOMINAL"
        assert packet.user_data["SWE_OPS_FLAG"] == "NON-HVSCI"
