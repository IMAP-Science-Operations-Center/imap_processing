...
from imap_processing.ultra.l0.ultra_utils import ULTRA_PRI_1_EVENTS
from imap_processing.utils import packet_file_to_datasets


@pytest.mark.external_test_data
def test_image_raw_events_decom(xtce_path, priority_1_test_path, ccsds_path_extra):
    """This function reads validation data and checks that decom data
    matches validation data for image rate packet"""

    datasets_by_apid = packet_file_to_datasets(ccsds_path_extra, xtce_path)
    decom_ultra = process_ultra_events(
        datasets_by_apid[ULTRA_PRI_1_EVENTS.apid[0]], ULTRA_PRI_1_EVENTS.apid[0]
    )
    df = pd.read_csv(priority_1_test_path, index_col="MET")
