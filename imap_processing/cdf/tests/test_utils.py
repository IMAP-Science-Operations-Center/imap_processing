...
def test_write_cdf(tmp_path):
    ...
    ds["Epoch"].attrs = ConstantCoordinates.EPOCH

    fname = write_cdf(ds, description="test-description")
    assert fname.exists()
    ...
    fname = write_cdf(ds, description="test-description", directory=tmp_path)
    assert fname.exists()
    assert fname.name == "imap_test_l1_test-description_20100101_v01.cdf"
    # Created automatically for us
    dir_structure = fname.parts[-5:-1]
