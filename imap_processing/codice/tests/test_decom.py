...
""" 1. The binary file is read using the read_binary_file() function.
    2. The index of a packet with the target APID (in this case, APID 96) is found using 
    the find_packet_with_apid() function.
    3. If a packet with the target APID is found, the code extracts the packet header and creates a
    dictionary of the CCSDS header fields using the extract_ccsds_header() function.
