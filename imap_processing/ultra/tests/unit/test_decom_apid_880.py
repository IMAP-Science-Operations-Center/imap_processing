...
def test_ultra_apid_880(decom_test_data, xtce_aux_test_path):
    ...
    for packet in decom_test_data:
        ...
        assert df.loc[time].SpinStartSeconds == packet.data["TIMESPINSTART"].derived_value
        assert df.loc[time].SpinStartSubseconds == packet.data["TIMESPINSTARTSUB"].derived_value
