...
def estimate_power_law_slope(
    ...
    gamma[central_valid] = (
        (right_log_fluxes - left_log_fluxes) / (right_log_energies - left_log_energies)
    )[central_valid]
    ...
    if uncertainties is not None:
        ...
        delta_gamma = np.sqrt(extended_rel_unc_sq[:-2] + extended_rel_unc_sq[2:]) / (
...
class PowerLawFluxCorrector:
    ...
    def predictor_corrector_iteration(
        ...
        gamma_initial, _ = estimate_power_law_slope(observed_fluxes, energies)
        ...
        for _iteration in range(max_iterations):
            ...
            # Predictor step
            gamma_pred, _ = estimate_power_law_slope(source_fluxes_n, energies)
            ...
            # Corrector step
            gamma_corr, _ = estimate_power_law_slope(source_fluxes_half, energies)
