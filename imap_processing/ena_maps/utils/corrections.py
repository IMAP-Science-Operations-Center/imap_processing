...
class PowerLawFluxCorrector:
    ...
    def __init__(self, coeffs_file: str | Path):
        ...
        self._load_coefficients(coeffs_file)

    def _load_coefficients(self, coeffs_file: str | Path) -> None:
        """
        Load ESA transmission scale factor coefficients from CSV file.

        Parameters
        ----------
        coeffs_file : str or Path
            Location of CSV file with coefficients.
        """
        self.eta_coeffs_df = pd.read_csv(coeffs_file, index_col="esa_step")
    ...
    def eta_esa(self, k: np.ndarray, gamma: np.ndarray) -> np.ndarray:
        ...
        for i, esa_step in enumerate(k):
            # np.polyval expects polynomial coeffs in reverse order
            eta[i] = np.polyval(
                self.eta_coeffs_df.loc[esa_step, coeff_columns].values[::-1], gamma[i]
            )
    ...
    def estimate_power_law_slope(
        ...
        interior_fallback = (
            need_fallback
            & (np.arange(n_levels) > 0)
            & (np.arange(n_levels) < n_levels - 1)
        )
