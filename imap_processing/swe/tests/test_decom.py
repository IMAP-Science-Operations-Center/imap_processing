...
class TestSwePacketDecom(unittest.TestCase):
    ...
    def test_total_packets_in_data_file(self):
        total_packets = 23
        self.assertEqual(len(self.data_packet_list), total_packets)
    ...
    def test_ccsds_header(self):
        ...
        self.assertListEqual(ccsds_header_keys, list(ccsds_header.keys()))
    ...
    def test_ways_to_get_data(self):
        ...
        self.assertEqual(data_value_using_key, data_value_using_list)
    ...
    def test_enumerated_value(self):
        ...
        if first_packet_data[f"{parameter_name}"].raw_value == 1:
            self.assertEqual(first_packet_data[f"{parameter_name}"].derived_value, "NOMINAL")
        if first_packet_data[f"{parameter_name}"].raw_value == 0:
            self.assertEqual(first_packet_data[f"{parameter_name}"].derived_value, "NOT_NOMINAL")
