...
def xtce_ialirt_path():
    ...
    return imap_module_directory / 'ialirt' / 'packet_definitions' \
        / "ialirt.xml"
...
def binary_packet_path():
    ...
    test session.
    """
    packet_path = imap_module_directory / 'ialirt' / 'tests' / 'test_data' / 'l0' / \
        'IALiRT Raw Packet Telemetry.txt'

    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        binary_file_path = Path(tmp_file.name)

    with open(packet_path, 'r') as text_file, open(binary_file_path, 'wb') as binary_file:
...
def empty_binary_packet_path():
    ...
    test session.
    """
    empty_packet_path = imap_module_directory / 'ialirt' / 'tests' / 'test_data' / 'l0' / \
        'Empty IALiRT Raw Packet Telemetry.txt'

    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        binary_file_path = Path(tmp_file.name)

    with open(empty_packet_path, 'r') as text_file, open(binary_file_path, 'wb') as binary_file:
        for line in text_file:
            if not line.startswith("#"):
                # Split the line by semicolons
                # Discard the first value since it is only a counter
...
def test_generate_xarray(binary_packet_path, xtce_ialirt_path, decom_packets_data):
    ...
    for instrument in instruments:
        ...
        assert instrument_list_set.issubset(packet_data_keys_set) == True
    ...
    for instrument in instruments:
        ...
        assert len(
            xarray_data[instrument].coords[dimension_name].values) == 32
